...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      count: pictograms?.length || 0,
    });
  } catch (error) {
    console.error("Custom pictograms list error:", error);
    return NextResponse.json(
      {
        error: "서버 오류가 발생했습니다.",
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (prompt.trim().length > 500) {
      return NextResponse.json({ error: "프롬프트는 500자 이내로 입력해주세요." }, { status: 400 });
    }
    ...
    // 데이터베이스에 저장
    const { data: pictogram, error } = await supabase
      .from("custom_pictograms")
      .insert({
        user_id: user.id,
        prompt: prompt.trim(),
    ...
    console.error("Custom pictogram create error:", error);

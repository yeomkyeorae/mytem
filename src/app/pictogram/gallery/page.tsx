...
export default function PictogramGalleryPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/pictograms/custom/${id}`, {
        method: "DELETE",
      });
    ...
    <>
      ...
      <main className="min-h-screen bg-black text-white pt-24 pb-16 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          {/* 빈 상태 */}
          {!isLoading && pictograms.length === 0 && !error && (
              ...
              {pictograms.map((pictogram) => (
                ...
                >
                  ...
                  <div className="p-4 pt-0 border-t border-white/10">
                    <p className="text-sm text-white truncate" title={pictogram.prompt}>
                      {pictogram.prompt}
                    </p>
                    <p className="text-xs text-white/40 mt-1">
                      {formatDate(pictogram.created_at)}
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    disabled={deletingId === pictogram.id}
